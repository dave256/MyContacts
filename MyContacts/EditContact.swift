...
final class EditContactModel: ObservableObject {
    ...
    var isEditing: Bool {
        if case Destination.edit? = destination {
            return true
        } else {
            return false
        }
    ...
    init(parentModel: ContactSaver? = nil, destination: Destination? = nil, contact: Contact) {
...
struct EditContact: View {
    ...
    var body: some View {
        VStack {
            IfCaseLet($model.destination, pattern: /EditContactModel.Destination.edit) { $field in
                EditContactForm(contact: $model.draftContact, startingFocus: field)
...
struct EditContact_Previews: PreviewProvider {
    ...
    static var previews: some View {
        NavigationStack {
            EditContact(model: EditContactModel(destination: .edit(.first), contact: .mock))
        }
    }
}
